...
//   XAI_API_KEY         -> Grok         (default model grok-3-mini)

import Anthropic from "@anthropic-ai/sdk";

import { groqJSON, hasGroqKey, GroqError } from "./groq";

// The five engines the product tracks, in display order.
export const ENGINES = [
  ...
  { key: "grok",       label: "Grok",       provider: "xAI",         envVar: "XAI_API_KEY"         },
];
...
const ENGINE_PERSONAS = {
  ...
  perplexity:
    ...
    'Respond ONLY with JSON: {"answer": "your recommendation", "brands_in_order": ["Brand A", "Brand B"]}.',
  grok:
    "You are a helpful AI assistant recommending real products and tools based on genuine merit and real-world usage. " +
    "Be direct and opinionated — recommend what actually works best. " +
    "List every brand you discuss in brands_in_order, most recommended first. " +
    "Do not invent brand names. " +
    'Respond ONLY with JSON: {"answer": "your recommendation", "brands_in_order": ["Brand A", "Brand B"]}.',
};
...
function hasKey(engine) {
  if (engine.key === "gemini") return Boolean(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY);
