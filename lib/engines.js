...
// Run one engine: real provider if its key is set, else the shared Groq
// fallback. `groqFallback` is a lazy, memoized single Groq call shared across
// every non-live engine — this avoids firing N parallel Groq requests (which
// trip the free-tier rate limit) and avoids pretending each unconfigured
// engine produced an independent answer.
async function runEngine(engine, prompt, brand, groqFallback) {
  ...
  try {
    if (live) {
      ...
      parsed = await groqFallback();
      backend = "Groq (fallback)";
    }
  } catch (err) {
    // If the real provider fails, degrade to the shared Groq result rather
    // than dropping the engine entirely.
    try {
      parsed = await groqFallback();
      backend = "Groq (fallback)";
      error = err instanceof Error ? err.message : "provider error";
    } catch (groqErr) {
      return engineError(engine, live, groqErr);
    }
  }
...
function engineError(engine, live, err) {
  return {
    key: engine.key,
    label: engine.label,
    provider: engine.provider,
...
export async function checkVisibilityAcrossEngines({ prompt, brand }) {
  // One shared Groq call, computed at most once and reused by every engine
  // that needs to fall back.
  ...
  const groqFallback = () => {
    ...
    if (!groqPromise) {
      groqPromise = groqJSON({ system: SYSTEM_PROMPT, user: prompt, maxTokens: 1500 });
    }
    return groqPromise;
  };

  const engines = await Promise.all(ENGINES.map((e) => runEngine(e, prompt, brand, groqFallback)));
