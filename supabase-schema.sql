...
-- webhook, which uses the service-role key). The RLS update policy above lets a
-- user update their own profile row for legit fields (brand, onboarding, etc.),
-- but without this trigger they could also self-grant a paid plan from the
-- browser. The trigger freezes billing columns for everyone except service_role.
